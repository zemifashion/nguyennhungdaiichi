...
class ToolsController extends AbstractMenuPage {
    ...
    private function handleSavePostRequest($data) {
        ...
        if ($postId) {
            ...
            $result = sprintf($msg . ' ' . _wpcc('You can check it here') . ': <a href="%1$s" target="_blank">%1$s</a>', $postUrl);
    ...
    private function handleRecrawlPostRequest($data) {
        if (!isset($data["_wpcc_tools_recrawl_post_id"])) return null;
    ...
    private function handleDeleteUrlsRequest($data) {
        if(!isset($data["_wpcc_tools_safety_check"])) {
            return _wpcc('You did not check the safety checkbox.');
        }

        if (!$data["_wpcc_tools_safety_check"] || !isset($data["_wpcc_tools_site_id"]) || !isset($data["_wpcc_tools_url_type"])) {
            return null;
        }
        ...
        if($urlTuple && $urlTuple->saved_post_id) {
            ...
            if($postData) {
                ...
                Informer::add(Information::fromInformationMessage(
                    ...
                    _wpcc("Post ID") . ": {$urlTuple->saved_post_id}",
