...
class SchedulingService {
    ...
    // TODO: A tool to trigger scheduling manually. When the scheduling is triggered, the plugin starts to crawl posts.
    // After crawling a post is finished, the plugin makes another request to the server to trigger another crawling
    // operation. By this way, each crawling will take place in another run. Hence, no timeout will occur. The user
    // should be able to enter how many posts he/she wants from the target site. When triggering another crawl operation,
    // the plugin sends the number of posts remaining to be crawled as well. By this way, we can track how many posts
    // are left and stop when they are all crawled. Only problem is to decide where and in what logic to get post URLs.
    // Maybe we can trigger URL collection event when we are out of post URLs.

    // TODO: Running schedules in the background asynchronously
