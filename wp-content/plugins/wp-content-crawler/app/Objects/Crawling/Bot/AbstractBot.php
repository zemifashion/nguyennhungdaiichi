...
abstract class AbstractBot {
    ...
    public function __construct($settings, $siteId = null, $useUtf8 = null, $convertEncodingToUtf8 = null) {
        ...
        $this->convertEncodingToUtf8    = $convertEncodingToUtf8 !== null   ? (bool) $convertEncodingToUtf8 : $this->getSettingForCheckbox("_wpcc_convert_charset_to_utf8");
     ...
     */
    public function makeInitialReplacements($crawler, $findAndReplaces = null, $applyGeneralReplacements = false) {
     ...
     *                           values
     */
    public function applyFindAndReplaceInElementAttributes(&$crawler, $optionKey) {
     ...
     */
    public function applyFindAndReplaceInElementHTML(&$crawler, $optionKey) {
        $data = $this->getSetting($optionKey);
        if(!$data) return;
    ...
    public function findAndReplaceInElementHTML(&$crawler, $selectors = [], $find, $replace, $regex = false) {
        ...
        foreach ($selectors as $selector) {
            ...
            try {
                $crawler->filter($selector)->each(function ($node, $i) use (&$find, &$replace, &$regex) {
                    /** @var Crawler $node */
