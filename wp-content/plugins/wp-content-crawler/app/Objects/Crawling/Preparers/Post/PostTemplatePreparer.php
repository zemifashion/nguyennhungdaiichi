...
class PostTemplatePreparer extends AbstractPostBotPreparer {
    ...
    private function applyOptionsBoxTemplates(&$shortCodeValueMap) {
        ...
        // Custom post meta
        $this->postData->setCustomMeta($this->applyShortCodesAndFindReplaces($this->postData->getCustomMeta(), $map, $frForMedia, "data"));

        // Custom post taxonomies
        $this->postData->setCustomTaxonomies($this->applyShortCodesAndFindReplaces($this->postData->getCustomTaxonomies(), $map, $frForMedia, "data"));
    ...
    private function prepareMainTitleShortCodeValue() {
        $templatePostTitle = $this->bot->getSetting('_post_template_title', '[' . ShortCodeName::WCC_MAIN_TITLE . ']');

        // If there is no template, stop.
        if(!$templatePostTitle) return;

        $this->replaceShortCode($templatePostTitle, ShortCodeName::WCC_MAIN_TITLE, $this->postData->getTitle() ? $this->postData->getTitle() : '');
    ...
    private function prepareMainExcerptShortCodeValue() {
        $templatePostExcerpt = $this->bot->getSetting('_post_template_excerpt', '[' . ShortCodeName::WCC_MAIN_EXCERPT . ']');
        ...
        $this->replaceShortCode($templatePostExcerpt, ShortCodeName::WCC_MAIN_TITLE, $this->postData->getTitle() ? $this->postData->getTitle() : '');
    ...
    private function prepareMainGalleryShortCodeValue() {
        $templateGalleryItem = $this->bot->getSetting('_post_template_gallery_item', '[' . ShortCodeName::WCC_GALLERY_ITEM_URL . ']');
    ...
    private function prepareMainTemplate(&$shortCodeValueMap) {
        $templateMain                        = $this->bot->getSetting('_post_template_main', '[' . ShortCodeName::WCC_MAIN_CONTENT . ']');
